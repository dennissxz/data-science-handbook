...
$$
\min _{i} \frac{1}{|\boldsymbol{w}|} y_{i}\left(\boldsymbol{w}^{T} \mathbf{x}_{i}+b\right)
$$

The objective of SVM is to find a hyperplane $\boldsymbol{w}^{T} \mathbf{x}_{i}+b = 0$ that separates two types of points and maximizes the margin.

$$
\max _{\boldsymbol{w}, b}\left\{\min _i \frac{1}{\|\boldsymbol{w}\|} y_{i}\left(\boldsymbol{w}^{T} \mathbf{x}_{i}+b\right)\right\}
$$
...
$$
\max _{\boldsymbol{w}, b} \frac{1}{\|\boldsymbol{w}\|} \left\{\min _i  y_{i}\left(\boldsymbol{w}^{T} \mathbf{x}_{i}+b\right)\right\}
$$

To classify a point with coordinates $\boldsymbol{x}$, we use the solution $\boldsymbol{w}^*$ and $b^*$.

- If $\boldsymbol{w}^{* \top} \boldsymbol{x} + b^* > 0$ then $\hat{y_i} = 1$
- If $\boldsymbol{w}^{* \top} \boldsymbol{x} + b^* < 0$ then $\hat{y_i} = -1$
...
$$
\min _{i} y_{i}\left(\boldsymbol{w}^{T} \mathbf{x}_i + b\right)=1
$$
...
\text {s.t.}&\quad \min_{i} y_{i}\left(\boldsymbol{w}^{T} \mathbf{x}_i+b\right)=1\\
\end{align}$$
...
\text {s.t.}&\quad y_{i}\left(\boldsymbol{w}^{T} \mathbf{x}_i+b\right)\ge1\\
\end{align}$$
...
$$\begin{align}
&& \frac{\partial}{\partial \boldsymbol{w}} L\left(\boldsymbol{w}, b, \boldsymbol{\lambda}\right) &= \boldsymbol{w}-\sum_{i=1}^{n} \lambda_{i} y_{i} \mathbf{x}_{i}\overset{\text{set}}{=} 0 \\
&\Rightarrow & \boldsymbol{w} &= \sum_{i=1}^{n} \lambda_{i} y_{i} \mathbf{x}_{i}  \\
...
$$\begin{align}
\max _{\boldsymbol{\lambda}} \,  &\quad \sum_{i=1}^{n} \lambda_{i}-\frac{1}{2} \sum_{i=1}^{n} \sum_{j=1}^{n}\lambda_{i} \lambda_{j} y_{i} y_{j} \mathbf{x}_{i} \mathbf{x}_{j} \\
...
\text { s.t. } & y_{i}\left(\boldsymbol{w}^{T} \mathbf{x}_{i}+b\right) \geq 1
\end{array}
\end{equation}
$$
...
$$
y_{i}\left(\boldsymbol{w}^{T} \mathbf{x}_{i}+b\right) \geq 1
$$
...
1, & \text { if } y_{i}\left(\boldsymbol{w}^{T} \mathbf{x}_{i}+b\right) < 1 \\
0, & \text { otherwise }
\end{array}\right.
\end{equation}
$$

A more reasonable loss is to measure the actual deviation from the constrain, i.e., $1 - y_{i}\left(\boldsymbol{w}^{T} \mathbf{x}_{i}+b\right)$, for those who violates the constraint, i.e.  $y_{i}\left(\boldsymbol{w}^{T} \mathbf{x}_{i}+b\right) < 1$. Overall, the loss function is


$$
R(\boldsymbol{w} ,b, \boldsymbol{x}_i ,y_i) = \max \left\{ 0, 1 - y_{i}\left(\boldsymbol{w}^{T} \mathbf{x}_{i}+b\right)  \right\}
$$
...
Let $\xi_{i}=\max \left\{0,1-y_{i}\left(\mathbf{w}^{T} x_{i}+w_{0}\right)\right\}$, then the objective is
...
$$\begin{align}
\underset{\boldsymbol{w}, b}{\min }&\quad \sum_{i=1}^{N} \alpha_{i}-\frac{1}{2} \sum_{i, j=1}^{N} \alpha_{i} \alpha_{j} y_{i} y_{j} \mathbf{x}_{i} \mathbf{x}_{j}    \\
